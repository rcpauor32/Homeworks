...
bool PhysBody::Contains(int x, int y) const
{
	...
	while (fixtures != nullptr) {
		...
		ret = shape->TestPoint(transform, { PIXEL_TO_METERS(x), PIXEL_TO_METERS(y) });
...
int PhysBody::RayCast(int x1, int y1, int x2, int y2, float& normal_x, float& normal_y) const
{
	...
	RInput.p1 = p1;
	RInput.p2 = p2;
	RInput.maxFraction = 1;

	b2RayCastOutput ROutput;

	float dist = INFINITE;

	b2Fixture* fxt = body->GetFixtureList();
	b2Shape* shape;
	while (fxt != nullptr) {
		shape = fxt->GetShape();
		if (shape->RayCast(&ROutput, RInput, body->GetTransform(), 0)) {
			float distance = ROutput.fraction * (RInput.p2 - RInput.p1).Length();
			if (distance < dist) {
				normal_x = ROutput.normal.x;
				normal_y = ROutput.normal.y;
				dist = distance;
			}
		}

		if (dist < INFINITE) {
			ret = METERS_TO_PIXELS(dist);
		}
