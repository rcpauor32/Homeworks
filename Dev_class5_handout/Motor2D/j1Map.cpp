...
void j1Map::Draw()
{
	...
	SDL_Rect toDraw;
	toDraw.h = data.tile_height;
	toDraw.w = data.tile_width;
	
	int tileId;
	int Idit = 0;

	// TODO 5: Prepare the loop to draw all tilesets + Blit
	for (int y = 0; y < data.layers.end->data->height; y++) {
		for (int x = 0; x < data.layers.end->data->width; x++) {
			tileId = data.layers.end->data->data[Idit++];
			toDraw.x = ;
			toDraw.y = ;
			App->render->Blit(data.tilesets.end->data->texture, x * data.tile_width, y * data.tile_height, );
		}
	}
...
bool j1Map::Load(const char* file_name)
{
	...
	for (layer_node = map_file.child("map").child("layer"); layer_node && ret; layer_node = layer_node.next_sibling("layer")) {
		...
		if (ret == true) {
			ret = LoadLayer(map_file, maplayer);
		}
...
bool j1Map::LoadLayer(pugi::xml_node & node, MapLayer * layer)
{
	...
	if (layer_node == NULL) {
		LOG("Error parsing layer: Cannot find 'layer' tag.");
		ret = false;
	}
	else {
